...
import {pagingQueryTagListAPI} from "../../apis/TagManagement/pagingQueryTagListAPI.tsx";
import {
    setTagListCountOfPage,
    setTagListCurrentPage,
    setTagListData, setTagListFlush,
} from "../../store/modules/tagListSlice.tsx";
import {deactivateTagAPI, deleteTagAPI, enableTagAPI} from "../../apis/TagManagement/tagOption.tsx";
...
const TagList: React.FC<TableComponentProps> = ({rowSelection}) => {
    ...
    const paginations: TablePaginationConfig = {
        ...
        total: tagList.countOfPage,
        onChange: (pageNumber) => {
            dispatch(setTagListCurrentPage(pageNumber));
            dispatch(setTagListFlush());
        },
        pageSize: 7,
    ...
    useEffect(() => {
        const pagingQueryTagList = async () => {
            try {
                const res = await pagingQueryTagListAPI(tagList.currentPage, 7);
                dispatch(setTagListData(res.data.data.tagList));
                dispatch(setTagListCountOfPage(res.data.data.countOfPage));
            } catch (error) {
            }
        }
